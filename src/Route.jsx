...
function App() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Agar backend se error message aata hai toh wo dikhayega
    ...
    <div className="bg-gradient-to-r from-[#e9b7ce] to-[#d3f3f1] flex justify-center items-center min-h-screen p-4">
      ...
      >
        ...
        <div className="flex justify-center w-full md:w-1/2 mb-6 md:mb-0">
          <img src={img1} alt="login" className="w-[200px] md:w-[300px] object-contain" />
        </div>
          
        <form className="w-full md:w-1/2 md:ml-10" onSubmit={handleLogin}>
          ...
          <div className="mb-4 text-right">
            <Link to="/forgetpassword" size="sm" className="text-sm text-blue-600 hover:text-blue-800 hover:underline transition-all">
              Forgot Password?
            </Link>
          </div>

          <button className="w-full bg-blue-500 hover:bg-blue-600 text-white p-2 rounded transition-colors" type="submit">
            Login
          </button>

          <Link to={"/register"} className="block text-center w-full bg-red-500 hover:bg-red-600 text-white p-2 rounded mt-[10px] transition-colors">
            Register
          </Link>
        </form>
